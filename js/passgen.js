...
function init() {
    ...
    if (window.localStorage.spritzState) {
        const spritzState = window.localStorage.spritzState
        if (N === JSON.parse(spritzState.length)) {
            Spritz.S = JSON.parse(spritzState)
        }
    }
...
function saveEntropy() {
    localStorage.setItem("spritzState", JSON.stringify(Spritz.S))
}
...
function randomScripps() {
    ...
    const scrippsText = "Need something to type? These words provide 256-bits of entropy:\n\n"
